...
const GetCurrentWeather = () => {
    ...
    useEffect(async()=>{
        if(locationKey){
            const {data} = await axios.get(`http://dataservice.accuweather.com/forecasts/v1/daily/1day/${locationKey}?apikey=${apiKey}&language=en-us&details=true&metric=true`)
          ...
          {(currentWeather)?(
            <div className = 'display-board'>
                <div className = 'location'> 
                    <span className="location__flag"><i className="fas fa-map-marker-alt"></i>  {currentLocation}</span>
                </div>
                <div className = 'weather'>
                    <div className = 'weather__main'>
                        <img src={`https://developer.accuweather.com/sites/default/files/${currentWeather.icon}-s.png`} alt="weather-icon" /><br/>
                        {currentWeather.weather}
                    </div>
                    <div className = 'weather__sub'>
                        <div><i className="fas fa-temperature-low"></i> {currentWeather.min}/<i className="fas fa-temperature-high"></i> {currentWeather.max}</div>
                        <div> <i className="fas fa-cloud-showers-heavy"></i> {currentWeather.rain} </div>
                        <div ><i className="fas fa-wind"></i> {currentWeather.wind}</div>
                    </div>
                </div>
